...
//表示件数用の変数
            ...
            for ($i = 1; $i <= $h; $i++) {
                if (!empty($_SESSION["search_result_tweet"][$i-1]["tweet_id"])) {
                    $_SESSION["search_result_tweet"][$i-1]["tweet_id"];
                    ...
                    if (!empty($_SESSION["search_result_tweet"][$i-1]["tweet_id"])) {
                        ...
                        echo '<input type="hidden" name="username" value="' . $_SESSION["search_result_tweet"][$i-1]["user_name"] . '">';
//                        echo '<a href="javascript:void(0)" onclick="document.img_tweetuser' . $i . '.submit();" style="text-decoration:none;"><FONT size=2><B>' . $dataset["user_name"] . '</B></FONT></a>';                        
                        ...
                        echo '<img src="./profile_img/' . $_SESSION["search_result_tweet"][$i-1]["user_id"] . '.jpg"'
                        ...
                        echo '<input type="hidden" name="username" value="' . $_SESSION["search_result_tweet"][$i-1]["user_name"] . '">';
                        echo '<a href="javascript:void(0)" onclick="document.name_tweetuser' . $i . '.submit();" style="text-decoration:none;"><FONT size=2><B>' . $_SESSION["search_result_tweet"][$i-1]["user_name"] . '</B></FONT></a>';
                        echo '</form>';
                        echo '<span style="margin-right: 6px;"></span>';

                        //投稿日の登録
                        echo '<span id="time">' . substr($_SESSION["search_result_tweet"][$i-1]["created_at"], 0, 10) . "</span><br>";
                        echo '<FONT size=4>' . $_SESSION["search_result_tweet"][$i-1]["tweet"] . "</FONT></br>";
                        ...
                        if (!empty($_SESSION["userid"])) {//ログアウトしているユーザーには出さないようにする。
                            ...
                            $iine = $dbh->prepare(
                                    ...
                                    . " and favorite_tweet_id = " . $_SESSION["search_result_tweet"][$i-1]["tweet_id"]
                            ...
                            if ($result["fnum"] > 0) {
                                ...
                                echo '<input type="hidden" name="tweetid" value=' . $_SESSION["search_result_tweet"][$i - 1]["tweet_id"] . '>';
                                ...
                                echo '&nbsp;&nbsp;';
                                echo $_SESSION["search_result_tweet"][$i - 1]["favorite"];
                                echo '&nbsp;&nbsp;&nbsp;';
                                echo '</form>';
                            } else {
                                echo '<form name=iine' . $i . ' action="edit.php" method="POST"  style="display:inline;">';
...
//つぶやき主のみ削除ボタンを表示
                            ...
                            if ($_SESSION["userid"] == $_SESSION["search_result_tweet"][$i-1]["user_id"]) {
                                ...
                                echo '<input type="hidden" name="focus_id" value=' . $i . '>'; //実験
